...
def main():
    """Основная функция для визуализации распределения оценок по странам."""
    try:
        with sqlite3.connect('wines.db') as conn:
            query = '''
            SELECT country, points FROM wines WHERE country IN (
                SELECT country FROM wines GROUP BY country HAVING COUNT(*) > 1000
            );
            '''
            data = execute_query(conn, query)
            if data:
                df = pd.DataFrame(data, columns=['country', 'points'])
                visualize_data(df, 'Страна', 'Оценка (points)', 'Распределение оценок по странам', 'plots/boxplot_points_by_country.png')
    except sqlite3.Error as e:
